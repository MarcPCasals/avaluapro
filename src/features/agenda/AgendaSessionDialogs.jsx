...
export function AgendaSessionAdjustDialog({
  ...
  return (
    <Modal onClose={onClose} panelClassName="agenda-dialog agenda-adjust-dialog" size="lg" title="Reajustar la sessió">
      <nav className="agenda-adjust-tabs">
        <button className={action === 'session' ? 'active' : ''} onClick={() => setAction('session')} type="button"><CalendarX2 size={15} />Sessió</button>
        <button className={action === 'activity' ? 'active' : ''} disabled={editableItems.length === 0} onClick={() => setAction('activity')} type="button"><Edit3 size={15} />Activitat</button>
        <button className={action === 'continuation' ? 'active' : ''} disabled={editableItems.length === 0} onClick={() => setAction('continuation')} type="button"><ArrowRight size={15} />Continuació</button>
      </nav>

      {action === 'session' && <section className="agenda-adjust-panel">
      ...
      {action === 'activity' && item && <section className="agenda-adjust-panel">
      ...
      {action === 'continuation' && item && <section className="agenda-adjust-panel">
        <label>Activitat<select value={item.id} onChange={(event) => selectItem(event.target.value)}>{editableItems.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.title}</option>)}</select></label>
